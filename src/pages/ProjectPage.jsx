...
const ProjectPage = () => {
  ...
  const handleSessionSelect = (session) => {
    ...
    navigate(`/project/session/${session.id}`);
  };
  ...
  return (
    <div className='flex-1 flex min-h-0'>
      ...
      {!isMobile && (
        <div className={`h-full flex-shrink-0 border-r border-border bg-card transition-all duration-300 ${sidebarVisible ? 'w-80' : 'w-14'}`}>
          ...
          <Route
            ...
            element={
              <MainContent
                ...
                onNavigateToSession={(sessionId) => navigate(`/project/session/${sessionId}`)}
                onShowSettings={() => window.openSettings?.()}
